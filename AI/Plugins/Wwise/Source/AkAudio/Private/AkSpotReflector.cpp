...
void AAkSpotReflector::BeginPlay()
{
	...
	else
	{
		if (!AuxBusName.IsEmpty())
			m_uAuxBusID = AK::SoundEngine::GetIDFromString(TCHAR_TO_ANSI(*AuxBusName));
		else
...
#if WITH_EDITOR
	sourceInfo.uNumChar = strlen(TCHAR_TO_ANSI(*GetActorLabel()));
	name = *GetActorLabel();
#endif // WITH_EDITOR
	...
	AK::SpatialAudio::AddImageSource(sourceInfo, m_uAuxBusID, roomID, AK_INVALID_GAME_OBJECT, name);
}
...
void AAkSpotReflector::BeginDestroy()
{
	...
	{
		AK::SpatialAudio::RemoveImageSource((AkImageSourceID)(uint64)this, m_uAuxBusID);
	}
}